...
function loadSvelte(version) {
  if (!svelteCache.has(version)) {
    if (version === 'local') {
      svelteCache.set(
        version,
        import(/* webpackChunkName: "svelte" */ 'svelte'),
      )
    } else {
      svelteCache.set(
        version,
...
export async function init(version) {
  await Promise.all([
    import(/* webpackChunkName: "rollup" */ 'rollup/dist/rollup.browser.js').then(
      r => {
        global.rollup = r
      },
    ),
      ...
      plugins: [
        {
          ...
          transform(code, id) {
            ...
            if (stats) {
              if (
                Object.keys(stats.hooks).filter(hook => stats.hooks[hook])
                  .length > 0
              )
                info.usesHooks = true
